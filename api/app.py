...
def home():
    ...
    return jsonify({
        ...
        'endpoints': {
            '/': 'This documentation',
            '/predict': 'POST - Predict revenue (single game)',
            '/predict-batch': 'POST - Predict revenue for a batch of games',
...
def predict():
    """Predict revenue for a single game"""
    if model is None:
        return jsonify({
            'success': False,
...
def predict_batch():
    """
    Predict revenue for a batch of games in a single request.
    ...
    try:
        ...
        for g in games:
            df, genre = prepare_features(g)
            dfs.append(df)
            genres.append(genre)
            cleaned_inputs.append(g)

        full_df = pd.concat(dfs, ignore_index=True)

        # Batch prediction timing
        start = time.perf_counter()
        preds_log = model.predict(full_df)
        batch_latency_ms = (time.perf_counter() - start) * 1000

        preds = np.expm1(preds_log)

        results = []
        for g, genre, pred in zip(cleaned_inputs, genres, preds):
            revenue = float(pred)
            results.append({
                'genre': genre,
                'input_data': g,
                'predicted_revenue_rupees': revenue,
                'predicted_revenue_millions': round(revenue / 1e6, 2),
                'predicted_revenue_formatted': f"₹{revenue/1e6:.1f}M"
            })
        ...
        return jsonify({
            ...
            'approx_latency_per_game_ms': batch_latency_ms / len(results),
            'results': results
        })
